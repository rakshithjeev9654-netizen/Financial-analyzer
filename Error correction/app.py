...
from modules.ai_analysis import generate_rule_based_analysis, generate_genai_analysis, answer_financial_question
...
def _run_analysis_and_store(analysis_id, norm_df, company_name, period, parse_warnings):
    ...
    _analysis_store[analysis_id].update({
        ...
        "ratios": {cat: _ratio_list_to_dict(lst) for cat, lst in ratios.items()},
...
def confirm_mapping():
    ...
    for field_name in STANDARD_FIELDS:
        ...
        if override:
            ...
            continue
        if choice not in ("__keep__", "__skip__") and choice in raw_df.columns:
            mapping[field_name] = raw_df[choice]
        elif choice == "__skip__":
            mapping[field_name] = None
        else:
            mapping[field_name] = ext["fields"][field_name]["period_values"]

    norm_df = build_normalized_df(mapping)
...
def dashboard():
    ...
    return render_template(
        ...
        kpis=kpis, health_score=data["health_score"], ml_risk=data["ml_risk"], genai=data.get("genai")
    )
...
def generate_ai():
    ...
    data = _analysis_store[analysis_id]
    try:
        answer = generate_genai_analysis(data["company_name"], data["period"], data["ratios"], data["health_score"], data["ml_risk"])
        data["genai"] = {"status": "success", "text": answer}
    except Exception as exc:
        data["genai"] = {"status": "error", "text": str(exc)}
    return redirect(url_for("dashboard"))
...
def ask_ai():
    ...
    if q:
        try:
            ans = answer_financial_question(q, data["company_name"], data["period"], data["ratios"], data["health_score"], data["ml_risk"])
            data["genai"] = {"status": "success", "text": ans, "question": q}
        except Exception as exc:
            data["genai"] = {"status": "error", "text": str(exc), "question": q}
    return redirect(url_for("dashboard"))
...
def history():
    ...
    analyses = [
        {"id": aid, "company_name": d["company_name"], "period": d["period"], "is_current": aid == current_id}
        for aid, d in _analysis_store.items() if d.get("ready")
    ]
